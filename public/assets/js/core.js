...
//    }
    ...
    //**********************************BEGIN MAIN MENU********************************
    $('a.active').parents('ul.sub-menu').parent('li').addClass('open');
	$('a.active').parents('ul.sub-menu').show();
	
	$('.innerpage_sidebar li').not('.open').children('.sub-menu').hide();
	$('.innerpage_sidebar li').click(function(){
	$(this).addClass( "open");
	setTimeout(function(){
	$('.innerpage_sidebar li.open').children('.sub-menu').slideDown(2000);
	},200);
	});
